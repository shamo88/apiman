...
export const apiConfigFromRequest = (r: any, fallbackName: string): ApiConfig => {
...
export const apiConfigFromHttpSpec = (spec: any, requestName: string): ApiConfig => {
  const bt = (spec.body_type || 'none') as ApiConfig['bodyType'];
  const allowed: ApiConfig['bodyType'][] = ['none', 'form-data', 'x-www-form-urlencoded', 'json', 'xml', 'raw', 'binary'];
  const bodyType = allowed.includes(bt) ? bt : 'none';
  const mapKV = (arr: any[] | undefined) =>
    Array.isArray(arr)
      ? arr.map((h) => ({
          key: h.key || '',
          value: h.value || '',
      ...
      : [];
  const mapPair = (arr: any[] | undefined) =>
    Array.isArray(arr)
      ? arr.map((p) => ({
          key: p.key || '',
          value: p.value || '',
